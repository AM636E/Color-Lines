...
{
	int **labirint = new int *[ SQUARES_X + 2 ];
	int k = 0, l = 0;
	for( int i = 1; k < SQUARES_X; i ++ )
	{
		labirint[ i ] = new int[ SQUARES_Y + 2 ];
		l = 0;
		for( int j = 1; l < SQUARES_Y; j ++ )
		{
			if( field.GetSquare( k, l ++ ).isFilled )
				labirint[ i ][ j ] = WALL;
			else
				labirint[ i ][ j ] = WAY;
		}
	...
	labirint[ 0 ] = new int[ SQUARES_Y + 2 ];
	labirint[ SQUARES_X + 1 ] = new int[ SQUARES_Y + 2 ];

	for( int i = 0; i <= SQUARES_X + 1; i ++ )
	{
		labirint[ 0 ][ i ]		   = WALL;
   		labirint[ SQUARES_X + 1 ][ i ] = WALL;
		labirint[ i ][ 0 ]		   = WALL;
		labirint[ i ][ SQUARES_Y + 1 ] = WALL;
	}
